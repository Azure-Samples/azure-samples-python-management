...
SKU = 'Standard'
# Resource Group
GROUP_NAME = 'myInternalLoadBalancer1'
...
def run_example():
    ...
    # Test Load Balancer
    # Create NIC for VM to test
    nic_test_parameters = network_models.NetworkInterface(
        location=LOCATION,
        # enable_accelerated_networking=True,
        ip_configurations=[network_models.NetworkInterfaceIPConfiguration(
            name=NIC_IP_CONFIG_NAME,
    ...
    nic_test = network_client.network_interfaces.begin_create_or_update(
        GROUP_NAME,
        NIC_TEST_NAME,
        nic_test_parameters
    ).result()
